...
def certificate(request):
    ...
    if request.method == 'POST':
        uploaded_files = [
            request.FILES.get('certificate1'),
            request.FILES.get('certificate2'),
            request.FILES.get('certificate3')
        ]
        ...
        for uploaded_file in uploaded_files:
            if uploaded_file:
                save_path = os.path.join(settings.MEDIA_ROOT, 'certificates', uploaded_file.name)
...
def user_has_access(user, page_name):
    try:
        ...
        return page in page_lock.pages.all() and not page_lock.is_locked
